...
def fit_hill_climbing():
    ...
    def neighbors(theta):
        ...
        for i in range(num_neighbors):
            vector = np.array([delta*cos(angle+pi*i/4), delta*sin(angle+pi*i/4)])
            neighbors_list.append(theta + vector)
...
def fit_simulated_annealing():
    ...
    def random_neighbor(theta):
        ...
        anlge = random.uniform(-pi,pi)
        return theta + np.array([delta*cos(anlge), delta*sin(anlge)])
